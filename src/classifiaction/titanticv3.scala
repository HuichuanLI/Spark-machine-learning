...
object titanticv3 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ageCategoryMap = titanicDF
      ...
      .map(row => {
        // 如果Age为null，使用平均年龄替换
        val ageValue = if (row.get(0) == null) avgAge else row.getDouble(0)
        // 按照年龄范围划分不同段
        if (ageValue >= 61) {
          "old"
        } else if (ageValue >= 26) {
    ...
    val titanicRDD: RDD[LabeledPoint] = titanicDF.select(
      ...
      val ageCateg: String = {
        if(ageFeature >= 61){
          "old"
        }else if(ageFeature >= 26){
          "middle"
        }else if(ageFeature >= 16){
          "young"
        }else{
          "child"
        }
      ...
      ageFeatureArray(ageCategoryMapBroadcast.value(ageCateg).toInt) = 1.0
